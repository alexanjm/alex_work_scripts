...
def test_delimiters():
    #out1 = ultimate_parser.main()
    #out2 = ultimate_parser.main()
...
def test_manual_delimiter1():
    output = ultimate_parser.get_delimiter('tests/test_commas.txt', 'c')

    assert output[0] == ','


def test_manual_delimiter2():
    output = ultimate_parser.get_delimiter('tests/test_tabs.txt', 't')
...
def test_read_infile():
    column_contents = ultimate_parser.read_infile('tests/test_commas.txt', ',', 2, [[], [], []])
...
def test_write_outfile():
    ...
    output = ultimate_parser.write_outfile(None, ofile, cols_list, 2, True, ',', None)
...
def test_write_outfile_input_delims_columns_parse():
    ofile = 'tests/test_commas.txt.parsed'
    cols_list = [['alex', '1', '4'], ['ryan', '2', '5'], ['kimani', '3', '6']]
    output = ultimate_parser.write_outfile('3,1', ofile, cols_list, 2, True, ',', 'c')
...
def test_sort_ascending_no_header():
    ...
    sorted = [['zeke', 291.0, -9.0], ['kimani', 2.0, -4.0], ['alex', 1.0, 1.0], ['peter', 12.0, 92.0]]
...
def test_main(monkeypatch):
    monkeypatch.setattr(sys, "argv", ['ultimate_parser', 'tests/sorted_test_header.txt', '-p', '-hd', '-sd', '2'])
